...
====================================================================
 ...
 for  P(X_T > threshold)  under Geometric Brownian Motion
====================================================================
...
# Number of independent replications for statistics
N_REPLICATIONS = 10
# Paths used by simple Monte Carlo (same total budget as IS inference)
MC_PATHS_LIST  = [500, 5_000, 50_000, 200_000]

# ── analytical (true) probability at eps=1 ──
log_thresh    = np.log(threshold / X0)
mean_log_true = (mu - 0.5 * sigma**2) * T
std_log_true  = sigma * np.sqrt(T)
P_TRUE        = 1.0 - scipy_norm.cdf(log_thresh, loc=mean_log_true, scale=std_log_true)
print(f"\nAnalytical P(X_T > {threshold}) = {P_TRUE:.10f}")
print("=" * 70)
...
previous_model = None
level_estimates_diag = []   # collect per-level IS estimates during training
analytical_vals_diag = []   # collect per-level analytical values
...
for level, eps in enumerate(epsilon_schedule):
    ...
    # ── per-level IS estimate (inference only, no training) ──
    x_lev   = torch.full((n_paths, dim), X0, device=DEVICE)
    ...
    hit_lev = (x_lev > threshold).float()
    est_lev = torch.mean(hit_lev * lr_lev).item()
    level_estimates_diag.append(est_lev)

    ml = (mu - 0.5 * sigma**2 * eps) * T
    sl = sigma * np.sqrt(eps * T)
    analytical_vals_diag.append(1.0 - scipy_norm.cdf(log_thresh, loc=ml, scale=sl))

    print(f"  IS est = {est_lev:.6f}  |  Analytical = {analytical_vals_diag[-1]:.6f}")
...
# keep the trained model for inference
...
for rep in range(N_REPLICATIONS):
    ...
    eps = 1.0  # final level  (the real distribution)
    x      = torch.full((n_paths, dim), X0, device=DEVICE)
    log_lr = torch.zeros(n_paths, 1, device=DEVICE)

    for step in range(steps):
        t = torch.full((n_paths, 1), step * dt, device=DEVICE)
        u  = get_optimal_control(trained_model, t, x, eps)
        dw = torch.randn_like(x) * np.sqrt(dt)
        sig_x = gbm_diffusion(x)
...
for n_mc in MC_PATHS_LIST:
    ...
    for rep in range(N_REPLICATIONS):
        ...
        # Simulate GBM paths analytically:  X_T = X0 * exp((mu-0.5*sigma^2)*T + sigma*sqrt(T)*Z)
        Z   = np.random.randn(n_mc)
        X_T = X0 * np.exp((mu - 0.5 * sigma**2) * T + sigma * np.sqrt(T) * Z)
        est = np.mean(X_T > threshold)
    ...
    print(f"  MC  N={n_mc:>8,d}  |  mean={np.mean(mc_ests):.8f}  "
...
def diagnostics(estimates, times, label):
    ...
    rmse = np.sqrt(mse)
    rel_err   = abs(bias) / P_TRUE * 100
    ci_half   = 1.96 * std / np.sqrt(len(est))
    ci_lo     = mean - ci_half
    ci_hi     = mean + ci_half
    covers    = (ci_lo <= P_TRUE <= ci_hi)
...
for n_mc in MC_PATHS_LIST:
    r = mc_results[n_mc]
    d = diagnostics(r["estimates"], r["times"], f"MC  (N={n_mc:,d})")
    all_diag.append(d)
...
print(f"\n  True value:  P(X_T > {threshold}) = {P_TRUE:.10f}\n")
print(header)
print(sep)
for d in all_diag:
...
# Compare IS vs MC at same path count
mc_same = mc_results.get(n_paths) or mc_results[MC_PATHS_LIST[0]]
var_is = d_is["std"] ** 2
var_mc = diagnostics(mc_same["estimates"], mc_same["times"],
                     "tmp")["std"] ** 2
...
for n_mc in MC_PATHS_LIST:
    t = mc_results[n_mc]["times"]
    print(f"  MC (N={n_mc:>8,d}) time per run:  {t.mean():.4f} s  (std {t.std():.4f} s)")
...
fig.suptitle(
    ...
    f"P(X_T > {threshold}) = {P_TRUE:.8f}   |   GBM(μ={mu}, σ={sigma}, X₀={X0})",
...
ax.axhline(P_TRUE, color="green", ls="--", lw=2, label=f"True = {P_TRUE:.6f}")
...
# ── Additional plot: convergence across IS levels (using data from Phase 1) ──
fig2, ax2 = plt.subplots(figsize=(9, 5))

lvls = range(1, n_levels + 1)
ax2.semilogy(lvls, level_estimates_diag, "o-",  lw=2, ms=6, color=color_is,
             label="IS estimate (per level)")
ax2.semilogy(lvls, analytical_vals_diag, "s--", lw=2, ms=6, color="green",
             label="Analytical (per ε)")
ax2.axhline(P_TRUE, color="blue", ls=":", lw=1.5,
            label=f"True P (ε=1) = {P_TRUE:.6f}")
ax2.set_xlabel("Level")
ax2.set_ylabel("Probability")
ax2.set_title(f"NN-IS Convergence Across Annealing Levels\nP(X_T > {threshold})")
ax2.legend()
...
# Find which MC sample size achieves comparable or better accuracy
is_rmse = d_is["rmse"]
for n_mc in MC_PATHS_LIST:
    mc_rmse = diagnostics(mc_results[n_mc]["estimates"],
